...
export async function generateProjectDescription(projectInfo: string) {
  const prompt = `Based on my project '${projectInfo}', generate three levels of project involvement: High Impact, Medium Impact, and Baseline Impact. Each description should be 3-4 lines long and written from my perspective, highlighting the project's significance, technologies used, and personal contributions. The output should be an array of JSON objects, each containing 'impact_level' and 'description' fields. You can include <b>, <i>, <u>, <s>, <blockquote>, <ul>, <ol>, and <li> to enhance the descriptions. Provide realistic and specific details about the project's scope, challenges overcome, and key achievements. Do not use placeholders.`;
